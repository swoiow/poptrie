...
class TestPopTrie(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.test_data = [
            "1.0.1.0/24",  # 标准 IPv4
            "110.16.0.0/12",  # 跨字节 IPv4
            "192.168.1.0/24",  # 用于测试不匹配
            "240e::/18",  # 重点测试：非对齐 IPv6
            "2001:da8::/32",  # 教育网 IPv6
            "1.0.1.5/32",  # 重复添加 (测试合并)
            "240e:0:0:0:0:0:0:1/128"  # 子网被 240e::/18 覆盖
        ]
        for cidr in cls.test_data:
    ...
    def test_batch_check_packed(self):
        ...
        results = self.searcher.batch_check_packed(packed_v4, False)
        self.assertEqual(results, [True, False, True, False])
